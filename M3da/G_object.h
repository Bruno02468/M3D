...
class G_Object: public CCmdTarget {
		DECLARE_DYNAMIC(G_Object)
		DECLARE_DISPATCH_MAP()
		DECLARE_INTERFACE_MAP()
	public:
		//*****************manually added api functions********************

		BSTR GetTest();
		// Retrieves a SAFEARRAY of BSTRs wrapped in a VARIANT
		virtual HRESULT __stdcall API_GetObjectHeaders(VARIANT* pVariant);
		virtual HRESULT __stdcall API_GetObjectVars(VARIANT* pVariant);
		virtual HRESULT __stdcall API_PutObjectVars(VARIANT* pVariant);
		virtual BSTR __stdcall API_GetObjectName();
		//*****************************************************************
		G_Object* pParent;
		G_Object* next;
		G_Object* before;
...
class NCurve: public G_Object {
	...
	public:
		...
		virtual void Transform(C3dMatrix TMat);
		virtual C3dVector GetPt(double dU);
		virtual C3dVector GetStartPt();
		virtual C3dVector GetEndPt();
		virtual C3dVector GetDir(double dU);
		virtual C3dVector Get_Centroid();
...
class ME_Object: public G_Object {
	...
	public:
		...
		virtual HRESULT __stdcall API_PutObjectVars(VARIANT* pVariant);
		virtual BSTR __stdcall API_GetObjectName();
		virtual BSTR __stdcall API_GetFEMName();
		virtual IDispatch* __stdcall API_GetNodeByInd(LONG iNo);
		...
		int iNoRes; // No of Results Sets
		int iCurResSetDef; // Results set for deformation
		int iResValDef; // Results variable for deformation -1 for full vector
		int iCVarDef; // Posted def value
		int iCurResSet; // Results set for Contour
		int iResVal; // Results variable for Contour
		int iPostOpt; // Secondart variable for Contour (layer)
		int iCVar; // Posted contour value
		int iSecID; // Seconday Contur var not used iPostOpt
		int iCurResVecSet; // Results set for vector displat
		int iResVec; // Results vector to display
		int iSecVecID; // Results vector secondary option unused
		...
		void PostContourVals(ResSet* pRes, int iVar, int iOpt, float& fMax, float& fMin);
	...
	protected:
		IDispatch* API_CreateNode2(DOUBLE x, DOUBLE y, DOUBLE z);

		enum {
			dispidCreateNode2 = 38L
		};
};
...
class SweepF: public Sweep {
	...
	public:
		...
		virtual void GenMesh(int iDim, PSHELL* pS1, PSHELL* pS2, PSHELL* pS3, PSHELL* pS4, PBARL* pB1, PBARL* pB2);
		virtual void CutBack(BOOL bEA, BOOL bEB, ME_Object* MeshA, ME_Object* MeshB);
};
