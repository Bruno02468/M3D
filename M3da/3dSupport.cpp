...
C3dMatrix C3dVector::GetTMat() {
	...
	vX.Set(1.0, 0.0, 0.0);
	vY.Set(0.0, 1.0, 0.0);
	dotX = std::abs(this->Dot(vX));
	dotY = std::abs(this->Dot(vY));
	if (dotX < dotY)
		vT = vX;
	else
		vT = vY;
...
void Circumsphere(C3dVector* v0, C3dVector* v1, C3dVector* v2, C3dVector* v3, C3dVector* center, double* radius) {
	...
	const double determinant =
	    Row1.x * (Row2.y * Row3.z - Row3.y * Row2.z) - Row2.x * (Row1.y * Row3.z - Row3.y * Row1.z) + Row3.x * (Row1.y * Row2.z - Row2.y * Row1.z);
