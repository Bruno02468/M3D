...
class DBase: public CCmdTarget {
	...
	public:
		...
		Text* AddText(C3dVector vN, C3dVector vDir, C3dVector vInPt, CString inText, double dH);
		// debug only
		...
		CvPt_Object* AddPt1(int iLab, int iCol, double x, double y, double z);
		CvPt_Object* AddPt2(double x, double y, double z, int iLab);
		...
		NLine* AddLN1(C3dVector v1, C3dVector v2, int ilab);
		...
		BOOL Extents(ObjList* Curves, C3dMatrix TMat, C3dVector vO, double& dXMax, double& dYMax, double& dXMin, double& dYMin);
	...
	protected:
		...
		LONG __stdcall API_GetNo(void);
		LONG __stdcall API_GetNoOnSrn(void);
		IDispatch* __stdcall API_CreatePoint(LONG iLab, LONG iCol, DOUBLE x, DOUBLE y, DOUBLE z);
		IDispatch* __stdcall API_CreateLine(LONG iLab, LONG iCol, DOUBLE x1, DOUBLE y1, DOUBLE z1, DOUBLE x2, DOUBLE y2, DOUBLE z2);
		enum {
			dispidGetObject = 4L,
	...
	public:
	protected:
		void __stdcall API_AddNode(DOUBLE X, DOUBLE Y, DOUBLE Z, LONG ID, LONG COL);
		void __stdcall API_ReDrawWindow();
		IDispatch* __stdcall API_GetObject(LONG iNo);
		IDispatch* __stdcall API_GetOnSrnObject(LONG iNo);
		IDispatch* __stdcall API_GetActiveMesh();
};
